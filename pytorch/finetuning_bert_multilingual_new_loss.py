...
class TripletTextDataset(Dataset):
    def __init__(self, tokenizer, max_len=64):
        with open("data.csv", 'r') as f:
            reader = csv.reader(f)
            next(reader)
            self.triplets = [(row[0],row[1], row[2]) for row in reader]
        #self.triplets = NameMatchingDataset(data_csv="data.csv")
...
def main():
    ...
    with mlflow.start_run():
        for epoch in range(EPOCHS):
            ...
            torch.save(model.state_dict(), f"./models/bert_triplet_finetuned_e{epoch}_v3.pt")
            print(f"Epoch {epoch+1} Loss: {total_loss:.4f}")

    # Save model
    torch.save(model.state_dict(), "./models/bert_triplet_finetuned_v3.pt")
